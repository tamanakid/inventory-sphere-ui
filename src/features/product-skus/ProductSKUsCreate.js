...
function ProductSKUsCreate(props) {
    ...
    async function executeRequest() {
        const product = (productId == -1) ? null : productId;
        const request = {
            endpoint,
            method: 'POST',
    ...
    return (
        <Layout
            form={(<div>
                <div className="form__title">Product SKUs - Create</div>
                {isLoading ? <div className="form__field">Loading...</div> : <>
                    ...
                    <div className="form__field">
                        <label>Product</label>
                        <Selector id={productId} setId={setProductId} options={options['products'] ?? []} />
                    </div>
...
export default ProductSKUsCreate;