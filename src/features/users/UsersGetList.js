...
function UsersGetList(props) {
    ...
    async function executeRequest() {
        ...
        if (email) queryParams.name = email;
        if (firstName) queryParams['first_name'] = firstName;
        if (lastName) queryParams['last_name'] = lastName;
        if (locations.length) queryParams.locations = locations;
        if (role != -1) queryParams.role = role;
    ...
    return (
        <Layout
            form={(<div>
                <div className="form__title">Users - Get List</div>
